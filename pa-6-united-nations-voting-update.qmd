...
```{r}
unvotes |> 
  count(country, vote) |> #counting the votes and country
  group_by(country) |> #grouping the votes by country
  mutate(total = sum(n)) |> #making a new variable of the total sum of votes 
  mutate(prop_vote = n/total) |> #making a percentage?
  filter(country %in% c("United States", "Canada", 
                        "Germany", "France",
                        "Italy", "Japan",
                        "United Kingdom")) |> #Only using the votes for the countries specified
  ggplot(aes(x = country, y = prop_vote, 
             fill = vote)) + #creating a graph for each proportional vote
  geom_col(position = position_stack()) + #creating labels
  labs(x = "Group of Seven Countries",
       y = "Proportion of Votes",
       title = "Voting Record of the G7",
       fill = "Vote") + #creating color for votes specific to country
  theme_minimal() +  #graph theme
  scale_fill_viridis_d(end = 0.8) + #visual aesthetic 
  coord_flip() #flipping the x and y axis
```

> Describe what the graph above demonstrates above UN voting records for the G7 
It shows the proportion of votes (abstain, no, yes) within the top 7 countries
...
> This code outputs the topics that countries are voting on.
...
> The context. We do not know what issues or things that the coutries are voting on. 
...
> The above information gives us context clues on the proportions of voting from each country and one can make assumptions about which topics may have been highly voted on based on history and world events. 
...
> I would assume the countries match with the issues and the only thing that would not match is how each country voted.
...
```{r}
roll_calls |> 
  left_join(issues, by = "rcid") #were retaining all the data in issues dataset by the name rcid
```

```{r}
roll_calls |> 
  right_join(issues, by = "rcid") #were retaining all the data in the issues dataset by the name rcid but on the right
```

```{r}
roll_calls |> 
  full_join(issues, by = "rcid") #retains all cases in either dataset
```

```{r}
roll_calls |> 
  inner_join(issues, by = "rcid") #retains only cases in both datasets
```
...
roll_calls |> 
  ...
  #We telling R to define certain issues to mean the same thing to be counted
  ...
  #counts number of issues in per year
  ggplot(aes(x = year, y = n, group = issue)) + #what does this line do?
  #ggplot is the creating a graph where x axis is the year, and y is the number of votes, grouped by issue
  geom_line(aes(color = issue)) + #what does this line do?
  #Adds color each issue
  labs(x = "Year", y = "Number of Votes",
...
> I notice that there is a high number of votes between 1980 and 2000 regarding the issues, which coincides with the fall of the USSR in the 90s.
